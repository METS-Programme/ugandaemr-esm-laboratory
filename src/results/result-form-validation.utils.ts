...
function validateNumericValue(
  ...
  if (concept.hiAbsolute && numValue > concept.hiAbsolute) {
    return t
      ? t('valueTooHigh', `${concept.display} value is critically high (max: ${concept.hiAbsolute})`)
      : `${concept.display} value is critically high (max: ${concept.hiAbsolute})`;
  }

  if (concept.lowAbsolute && numValue < concept.lowAbsolute) {
    return t
      ? t('valueTooLow', `${concept.display} value is critically low (min: ${concept.lowAbsolute})`)
...
export function validateResults(
  ...
  if (concept.set && concept.setMembers?.length > 0) {
    ...
    if (filledMembers.length === 0) {
      errors.push(
        t ? t('panelMustHaveResult', 'Panel must have at least one result') : 'Panel must have at least one result',
      );
    }
  } else {
    ...
    if (!hasValue(value)) {
      errors.push(t ? t('testMustHaveValue', 'Test must have a value to save') : 'Test must have a value to save');
    }
  }
 ...
 */
export function validateObservationPayload(obsPayload: Record<string, unknown>): { valid: boolean; error?: string } {
  try {
